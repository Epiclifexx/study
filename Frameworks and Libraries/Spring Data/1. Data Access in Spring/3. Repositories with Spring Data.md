...
Now, the reason we're doing all this, is so that we can wait until Spring is actually donde building and ready to do work before we go and query the database, because that's very important, that the database is all set up by the time we run this code.
...
So now, we get all the rooms in the "rooms" variable using the find All method. And then, we can call the "for each" method using the variable "rooms". And for each interaction, we can call a method, like for example: `System.out::println`. So, our code should look like this now:
```java
